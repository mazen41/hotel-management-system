...
import { BedDouble, CalendarCheck, Crown, Filter, Hotel, LayoutDashboard, LogOut, Menu, PanelLeftClose, Plus, Search, Settings, UserRound, UsersRound, X } from 'lucide-react';
import { BedDouble, CalendarCheck, Crown, DoorOpen, Filter, Hotel, LayoutDashboard, LogOut, Plus, Search, Settings, UserRound, UsersRound } from 'lucide-react';
...
function DashboardShell({ children, path, navigate }) {
  ...
  return <div className={`app ${collapsed ? 'sidebarCollapsed' : ''} ${mobileOpen ? 'mobileNavOpen' : ''}`}>
    <aside className="sidebar" aria-label="Dashboard navigation">
      <div className="brand"><Hotel size={24} /><span>NoBeds Hotel OS</span></div>
      <nav>{NAV_ITEMS.map(({ path: itemPath, label, icon: Icon }) => <button key={itemPath} className={path === itemPath ? 'active' : ''} onClick={() => go(itemPath)} aria-current={path === itemPath ? 'page' : undefined}><Icon size={18} /><span>{label}</span></button>)}<button className="logout" onClick={() => { localStorage.removeItem('auth_token'); window.location.assign('/login'); }}><LogOut size={18} /><span>Logout</span></button></nav>
    </aside>
    ...
    <main className="main">
      <header className="topbar">
        <div className="topbarLeft">
          ...
          <button className="collapseButton" onClick={() => setCollapsed((value) => !value)} aria-label="Toggle collapsed sidebar"><PanelLeftClose size={18} /></button>
...
function ProtectedApp({ children }) {
  ...
  if (!hasToken && window.location.pathname !== '/login') {
    return <div className="authNotice"><div><Hotel size={34}/><p className="eyebrow">Authentication required</p><h1>Sign in to access the hotel dashboard</h1><p>This admin area is wired for protected API routes. Add an auth token to continue.</p><button onClick={() => window.location.assign('/login')}>Go to login</button></div></div>;
  }
  return children;
}
...
function GuestForm({ guest, onCancel, onSaved }) {
  const [form, setForm] = useState(guest || emptyGuest); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  async function submit(event) { event.preventDefault(); setSaving(true); setError(''); try { const payload = { ...form, vip_status: Boolean(form.vip_status), marketing_consent: Boolean(form.marketing_consent) }; const response = form.id ? await guestsApi.update(form.id, payload) : await guestsApi.create(payload); onSaved(response.data.data); } catch (err) { setError(err.response?.data?.message || 'Unable to save guest profile.'); } finally { setSaving(false); } }
...
function GuestsPage() {
  const [guests, setGuests] = useState([]); const [meta, setMeta] = useState({ current_page: 1, last_page: 1 }); const [selected, setSelected] = useState(null); const [editing, setEditing] = useState(null); const [loading, setLoading] = useState(false); const [filters, setFilters] = useState({ search: '', country: '', vip_status: '' });
  async function load(page = 1) { setLoading(true); const res = await guestsApi.list({ ...filters, vip_status: filters.vip_status || undefined, page, per_page: 10 }); setGuests(res.data.data); setMeta(res.data.meta || {}); setLoading(false); }
...
function ReservationForm({ reservation, guests, rooms, onCancel, onSaved }) {
  const [form, setForm] = useState(reservation || emptyReservation); const [availableRooms, setAvailableRooms] = useState([]); const [error, setError] = useState(''); const [checking, setChecking] = useState(false); const [saving, setSaving] = useState(false);
  const selectedRoom = rooms.find((room) => Number(room.id) === Number(form.room_id)); const selectedRoomType = selectedRoom?.room_type; const nights = nightsBetween(form.check_in_date, form.check_out_date); const subtotal = Number(selectedRoomType?.base_price || 0) * nights; const total = subtotal + Number(form.taxes || 0) + Number(form.fees || 0); const balance = Math.max(total - Number(form.paid_amount || 0), 0);
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  async function checkAvailability() { if (!form.check_in_date || !form.check_out_date) return; setChecking(true); setError(''); try { const res = await roomsApi.availability({ check_in: form.check_in_date, check_out: form.check_out_date, adults: form.adults, children: form.children }); setAvailableRooms(res.data.data); } catch (err) { setError(err.response?.data?.message || 'Unable to check availability.'); } finally { setChecking(false); } }
  async function submit(event) { event.preventDefault(); setSaving(true); setError(''); try { const payload = { ...form, guest_id: Number(form.guest_id), room_id: Number(form.room_id), room_type_id: Number(form.room_type_id || selectedRoom?.room_type_id), adults: Number(form.adults), children: Number(form.children || 0), taxes: Number(form.taxes || 0), fees: Number(form.fees || 0), paid_amount: Number(form.paid_amount || 0) }; const response = form.id ? await reservationsApi.update(form.id, payload) : await reservationsApi.create(payload); onSaved(response.data.data); } catch (err) { setError(err.response?.data?.message || Object.values(err.response?.data?.errors || {})?.[0]?.[0] || 'Unable to save reservation.'); } finally { setSaving(false); } }
...
const modules = [
  ...
  { id: 'roomTypes', label: 'Room Types', icon: Hotel },
...
function GuestForm({ guest, onCancel, onSaved }) {
  ...
  async function submit(event) { 
    ...
    try { 
      const payload = { 
        ...form, 
        vip_status: Boolean(form.vip_status), 
  ...
  return (
    <div className="modalBackdrop">
      <form className="guestForm modal" onSubmit={submit}>
        <div className="modalHeader">
          <div>
            <p className="eyebrow">Guest CRM</p>
            <h2>{form.id ? 'Edit guest' : 'Create guest'}</h2>
          </div>
        ...
        <div className="grid two">
          <label>Passport number<input value={form.passport_number || ''} onChange={(e) => update('passport_number', e.target.value)} /></label>
          <label>National ID<input value={form.national_id || ''} onChange={(e) => update('national_id', e.target.value)} /></label>
        </div>
        ...
        <div className="switches">
          <label><input type="checkbox" checked={Boolean(form.vip_status)} onChange={(e) => update('vip_status', e.target.checked)} /> VIP guest</label>
          <label><input type="checkbox" checked={Boolean(form.marketing_consent)} onChange={(e) => update('marketing_consent', e.target.checked)} /> Marketing consent</label>
        </div>
        <div className="actions">
          <button type="button" className="ghost" onClick={onCancel}>Cancel</button>
          <button disabled={saving}>{saving ? 'Saving…' : 'Save guest'}</button>
        </div>
      </form>
    </div>
  );
  ...
  return (
    <section className="profile">
      <div className="profileTop">
        <div className="avatar">{guest.first_name?.[0]}{guest.last_name?.[0]}</div>
        <div>
          <p className="eyebrow">Guest profile</p>
          <h2>{guest.full_name}</h2>
          <p>{guest.email || 'No email'} · {guest.phone || 'No phone'}</p>
        </div>
        {guest.vip_status && <span className="vip"><Crown size={14}/>VIP</span>}
      </div>
      <div className="profileGrid">
        <div><span>Location</span><strong>{[guest.city, guest.country].filter(Boolean).join(', ') || '—'}</strong></div>
        <div><span>Passport</span><strong>{guest.passport_number || '—'}</strong></div>
        <div><span>National ID</span><strong>{guest.national_id || '—'}</strong></div>
        <div><span>Marketing</span><strong>{guest.marketing_consent ? 'Consented' : 'Not opted in'}</strong></div>
      </div>
      <button className="wide" onClick={() => onEdit(guest)}>Edit profile</button>
      <div className="history">
        <div><p className="eyebrow">Reservation history</p><h3>Ready for reservations</h3></div>
        <p>Direct bookings, NoBeds OTA imports, billing activity, and repeat-guest analytics appear here as reservations are created.</p>
      </div>
    </section>
  );
}
...
function GuestsPage() {
  ...
  async function openGuest(guest) { 
    try {
      const res = await guestsApi.show(guest.id); 
      setSelected(res.data.data); 
    } catch (err) {
      console.error('Failed to show guest:', err);
    }
  }

  function saved(guest) { 
    setEditing(null); 
    ...
    <>
      <header className="hero">
        <div>
          ...
          <p>Scalable CRM profiles for direct guests, NoBeds OTA imports, loyalty, billing, and repeat-stay analytics.</p>
        </div>
        <button onClick={() => setEditing(emptyGuest)}><Plus size={16}/>New guest</button>
      </header>
      <section className="toolbar">
        <div className="search">
          <Search size={18}/>
          <input placeholder="Search name, email, phone, passport…" value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })}/>
        </div>
        <div className="filter">
          <Filter size={16}/>
          <input placeholder="Country" value={filters.country} onChange={(e) => setFilters({ ...filters, country: e.target.value })}/>
          <select value={filters.vip_status} onChange={(e) => setFilters({ ...filters, vip_status: e.target.value })}>
      ...
      <div className="content">
        <section className="tableCard">
          <div className="tableHeader">
            <h2>Guest list</h2>
            <span>{meta.total || 0} profiles</span>
          </div>
          {loading ? (
            ...
            <table>
              ...
              <tbody>
                {guests.map((guest) => (
                  <tr key={guest.id} onClick={() => openGuest(guest)} className={selected?.id === guest.id ? 'selected' : ''}>
                    <td><strong>{guest.full_name}</strong><small>{guest.passport_number || guest.national_id || 'No ID on file'}</small></td>
                    <td>{guest.email || '—'}<small>{guest.phone || '—'}</small></td>
                    <td>{[guest.city, guest.country].filter(Boolean).join(', ') || '—'}</td>
                    <td>{guest.vip_status ? <span className="vip"><Crown size={14}/>VIP</span> : <span className="standard">Standard</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <GuestProfile guest={selected} onEdit={setEditing}/>
      </div>
      {editing && <GuestForm guest={editing} onCancel={() => setEditing(null)} onSaved={saved}/>}
    </>
  );
}

function Badge({ value, type = 'status' }) { 
  return <span className={`badge ${type}-${String(value).replace('_', '-')}`}>{String(value || '—').replace('_', ' ')}</span>; 
}

function Pager({ meta, load }) { 
  return (
    <div className="pagination">
      <button disabled={(meta.current_page || 1) <= 1} onClick={() => load((meta.current_page || 1) - 1)}>Previous</button>
      <span>Page {meta.current_page || 1} of {meta.last_page || 1}</span>
      <button disabled={(meta.current_page || 1) >= (meta.last_page || 1)} onClick={() => load((meta.current_page || 1) + 1)}>Next</button>
    </div>
  ); 
}
...
function ReservationForm({ reservation, guests, rooms, onCancel, onSaved }) {
  ...
  async function checkAvailability() { 
    ...
    try { 
      ...
      setError(err.response?.data?.message || 'Unable to check availability.'); 
    } finally { 
      setChecking(false); 
    } 
  }
  ...
  async function submit(event) { 
    ...
    try { 
      const payload = { 
        ...
        room_type_id: Number(form.room_type_id || selectedRoom?.room_type_id), 
      ...
      setError(err.response?.data?.message || Object.values(err.response?.data?.errors || {})?.[0]?.[0] || 'Unable to save reservation.'); 
    } finally { 
      setSaving(false); 
    } 
  }
      ...
      <form className="guestForm modal" onSubmit={submit}>
        <div className="modalHeader">
          <div>
            ...
            <h2>{form.id ? 'Edit reservation' : 'Create reservation'}</h2>
          </div>
          <button type="button" onClick={onCancel}>×</button>
        </div>
        {error && <div className="error">{error}</div>}
        <div className="grid two">
          <label>Guest
            <select required value={form.guest_id} onChange={(e) => update('guest_id', e.target.value)}>
              <option value="">Select guest</option>
              {guests.map((guest) => <option key={guest.id} value={guest.id}>{guest.full_name}</option>)}
            </select>
          </label>
          <label>Source<input value={form.source} onChange={(e) => update('source', e.target.value)} placeholder="direct, walk_in, nobeds…" /></label>
        </div>
        <div className="grid four">
          <label>Check in<input required type="date" value={form.check_in_date} onChange={(e) => update('check_in_date', e.target.value)} /></label>
          <label>Check out<input required type="date" value={form.check_out_date} onChange={(e) => update('check_out_date', e.target.value)} /></label>
          <label>Adults<input required type="number" min="1" value={form.adults} onChange={(e) => update('adults', e.target.value)} /></label>
          <label>Children<input type="number" min="0" value={form.children} onChange={(e) => update('children', e.target.value)} /></label>
        </div>
        <button type="button" className="ghost availability" onClick={checkAvailability}>{checking ? 'Checking…' : 'Check room availability'}</button>
        <label>Room
          <select required value={form.room_id} onChange={(e) => { const room = rooms.find((item) => Number(item.id) === Number(e.target.value)); update('room_id', e.target.value); update('room_type_id', room?.room_type_id || ''); }}>
            <option value="">Select available room</option>
            {roomOptions.map((room) => <option key={room.id} value={room.id}>Room {room.room_number} — {room.room_type?.name || 'Room type'} (${room.room_type?.base_price || 0}/night)</option>)}
          </select>
        </label>
        <div className="grid two">
          <label>Status
            <select value={form.status} onChange={(e) => update('status', e.target.value)}>
              {['pending','confirmed','checked_in','checked_out','cancelled','no_show'].map((value) => <option key={value}>{value}</option>)}
            </select>
          </label>
          <label>Payment status
            <select value={form.payment_status} onChange={(e) => update('payment_status', e.target.value)}>
              {['unpaid','partially_paid','paid','refunded'].map((value) => <option key={value}>{value}</option>)}
            </select>
          </label>
        </div>
        <div className="grid three">
          <label>Taxes<input type="number" min="0" step="0.01" value={form.taxes} onChange={(e) => update('taxes', e.target.value)} /></label>
          <label>Fees<input type="number" min="0" step="0.01" value={form.fees} onChange={(e) => update('fees', e.target.value)} /></label>
          <label>Paid amount<input type="number" min="0" step="0.01" value={form.paid_amount} onChange={(e) => update('paid_amount', e.target.value)} /></label>
        </div>
        ...
        <label>Special requests<textarea value={form.special_requests || ''} onChange={(e) => update('special_requests', e.target.value)} /></label>
        <label>Internal notes<textarea value={form.internal_notes || ''} onChange={(e) => update('internal_notes', e.target.value)} /></label>
        <div className="grid two">
          <label>NoBeds / external reservation ID<input value={form.external_reservation_id || ''} onChange={(e) => update('external_reservation_id', e.target.value)} /></label>
          <label>Channel manager reference<input value={form.channel_manager_reference || ''} onChange={(e) => update('channel_manager_reference', e.target.value)} /></label>
        </div>
        <div className="actions">
          <button type="button" className="ghost" onClick={onCancel}>Cancel</button>
          <button disabled={saving}>{saving ? 'Saving…' : 'Save reservation'}</button>
        </div>
      </form>
    </div>
  );
...
function ReservationsPage() {
  const [reservations, setReservations] = useState([]); const [meta, setMeta] = useState({ current_page: 1, last_page: 1 }); const [guests, setGuests] = useState([]); const [rooms, setRooms] = useState([]); const [selected, setSelected] = useState(null); const [editing, setEditing] = useState(null); const [filters, setFilters] = useState({ search: '', status: '', payment_status: '', source: '' }); const [loading, setLoading] = useState(false);
  async function load(page = 1) { setLoading(true); const res = await reservationsApi.list({ ...filters, status: filters.status || undefined, payment_status: filters.payment_status || undefined, source: filters.source || undefined, page, per_page: 10 }); setReservations(res.data.data); setMeta(res.data.meta || {}); setLoading(false); }
  async function loadLookups() { const [guestRes, roomRes] = await Promise.all([guestsApi.list({ per_page: 100 }), roomsApi.list({ active: true })]); setGuests(guestRes.data.data); setRooms(roomRes.data.data); }
  useEffect(() => { load(1); }, [filters.search, filters.status, filters.payment_status, filters.source]); useEffect(() => { loadLookups(); }, []);
  async function openReservation(reservation) { const res = await reservationsApi.show(reservation.id); setSelected(res.data.data); }
...
function App() {
  ...
  return <ProtectedApp><DashboardShell path={path} navigate={navigate}>{pages[path] || pages['/dashboard']}</DashboardShell></ProtectedApp>;
}
...
createRoot(document.getElementById('root')).render(<App />);
  return (
    <section className="profile">
      <div className="profileTop">
        <div className="avatar">{reservation.reservation_number?.slice(-2)}</div>
        <div>
          <p className="eyebrow">Reservation</p>
          <h2>{reservation.reservation_number}</h2>
          <p>{reservation.guest?.full_name || 'Guest'} · Room {reservation.room?.room_number || '—'}</p>
        </div>
      </div>
      <div className="profileGrid">
        <div><span>Status</span><strong><Badge value={reservation.status}/></strong></div>
        <div><span>Payment</span><strong><Badge value={reservation.payment_status} type="pay"/></strong></div>
        <div><span>Dates</span><strong>{reservation.check_in_date} → {reservation.check_out_date}</strong></div>
        <div><span>Total / Balance</span><strong>${reservation.total_amount} / ${reservation.balance_due}</strong></div>
      </div>
      <button className="wide" onClick={() => onEdit(reservation)}>Manage reservation</button>
      <div className="history">
        <p className="eyebrow">Channel manager readiness</p>
        <h3>NoBeds sync prepared</h3>
        <p>External reservation ID and channel manager reference fields are stored for future NoBeds OTA import and calendar synchronization workflows.</p>
      </div>
    </section>
  );
}
...
function ReservationsPage() {
  ...
  async function openReservation(reservation) { 
    try {
      const res = await reservationsApi.show(reservation.id); 
      setSelected(res.data.data); 
    } catch (err) {
    ...
    <>
      <header className="hero">
        <div>
          ...
          <p>Create direct reservations, validate room availability, calculate totals, manage statuses, and prepare NoBeds OTA synchronization.</p>
        </div>
        <button onClick={() => setEditing(emptyReservation)}><Plus size={16}/>New reservation</button>
      </header>
      <section className="toolbar">
        <div className="search">
          <Search size={18}/>
          <input placeholder="Search reservation, guest, phone, email, NoBeds reference…" value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })}/>
        </div>
        <div className="filter reservationFilters">
          <Filter size={16}/>
      ...
      <div className="content">
        <section className="tableCard">
          <div className="tableHeader">
            <h2>Reservations</h2>
            <span>{meta.total || 0} bookings</span>
          </div>
          {loading ? (
            ...
            <table>
              ...
              <tbody>
                {reservations.map((reservation) => (
                  <tr key={reservation.id} onClick={() => openReservation(reservation)} className={selected?.id === reservation.id ? 'selected' : ''}>
                    <td><strong>{reservation.reservation_number}</strong><small>{reservation.source}</small></td>
                    <td>{reservation.guest?.full_name || '—'}<small>Room {reservation.room?.room_number || '—'}</small></td>
                    <td>{reservation.check_in_date} → {reservation.check_out_date}<small>{reservation.nights} nights</small></td>
                    <td><Badge value={reservation.status}/><small><Badge value={reservation.payment_status} type="pay"/></small></td>
                    <td><strong>${reservation.total_amount}</strong><small>Balance ${reservation.balance_due}</small></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <ReservationProfile reservation={selected} onEdit={setEditing}/>
      </div>
      {editing && <ReservationForm reservation={editing} guests={guests} rooms={rooms} onCancel={() => setEditing(null)} onSaved={saved}/>}
    </>
  );
}
...
function App() {
  ...
  const pages = { 
    dashboard: <PlaceholderPage title="Dashboard" description="KPI, recent activity, and occupancy trend widgets remain connected to the dashboard API."/>, 
    roomTypes: <PlaceholderPage title="Room Types" description="Manage inventory products, pricing, occupancy limits, and channel-manager mapping codes."/>, 
    rooms: <PlaceholderPage title="Rooms" description="Manage physical room inventory and use the availability endpoint for date-range searches."/>, 
    guests: <GuestsPage/>, 
    reservations: <ReservationsPage/>, 
    settings: <PlaceholderPage title="Settings" description="Maintain hotel profile, operational preferences, and future NoBeds configuration."/> 
  };
